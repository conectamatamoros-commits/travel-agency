...
function processSheet(ws: XLSX.WorkSheet, seccion?: string): object[] {
  ...
  for (let r = range.s.r; r <= range.e.r; r++) {
    ...
    const sec = seccion ?? txts.find(s =>
      ...
      !/^\d[\d\s\-\.]{5,}$/.test(s) &&
      s.length > 1 && !['L', 'M', 'S', 'null'].includes(s)
    )
  ...
  for (const sname of wb.SheetNames) {
    ...
    if (upper === 'CONTACTO') {
      ...
      for (const row of rows) {
        const txts = (row as unknown[]).map(v => String(v ?? '').trim())
          .filter(v => v && v !== 'null' && v.length > 2)
    ...
    if (upper.includes('ESPERA')) {
      const rows = XLSX.utils.sheet_to_json<unknown[]>(ws, { header: 1, defval: null })
      for (const row of rows) {
        const txts = (row as unknown[]).map(v => String(v ?? '').trim()).filter(v => v && v !== 'null')
        const n = txts.find(t => looksLikeName(t))
        if (n) allViajeros.push({ nombre: n, celular: txts.find(t => t.replace(/\D/g, '').length >= 7), personas: 1 })
      }
      continue
    }
...
export default function ImportarClient({ viajesExistentes, userId }: Props) {
  ...
  const addFiles = useCallback((newFiles: File[]) => {
    ...
    setFiles(prev => {
      ...
      const toAdd = valid.filter(f => !ex.has(f.name)).map(f => ({ file: f, status: 'pending' as const }))
      return [...prev, ...toAdd]
    })
  }, [])
  ...
  const processFile = useCallback(async (index: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFiles(prev => prev.map((f, i) => i === index ? {
          ...f, status: 'error' as const, message: result.error
        } : f))
      }
    } catch (err) {
      setFiles(prev => prev.map((f, i) => i === index ? {
        ...
        <div className="card p-5">
          ...
          <div className="space-y-2">
            ...
            {viajesExistentes.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-4">Sin viajes aún</p>
            )}
          </div>
        </div>
      </div>
