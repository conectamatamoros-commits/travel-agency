...
// Very simple name detection - string with space, no digits, length > 5
function looksLikeName(v: unknown): boolean {
  if (!v || typeof v !== 'string') return false
  const s = v.trim()
  if (s.length < 5 || !s.includes(' ')) return false
  if (/\d/.test(s)) return false
  const BAD = ['TOTAL','DOBLE','TRIPLE','CUADRUPLE','INDIVIDUAL','TRANSPORTE','KIT','GASTOS',
    'GNP','CANCHA','VIP','GENERAL','STAY','VIAJERO','NOMBRE','INFO','CONTACTO','ABONO',
    'TABLAS','HABITACI','ROOMLIST','DADO','BAJA','VUELOS','TRASLADOS','BOLETOS']
  return !BAD.some(k => s.toUpperCase().includes(k))
}

const TIPO_MAP: Record<string,string> = {
  'DOBLE':'Doble','TRIPLE':'Triple','CUADRUPLE':'Cuadruple','CUADRUPLE':'Cuadruple','INDIVIDUAL':'Individual'
}
const TALLAS = new Set(['XS','S','M','L','XL','2XL','XXL'])

function processSheet(ws: XLSX.WorkSheet, seccion?: string) {
  ...
  for (let r = range.s.r; r <= range.e.r; r++) {
    ...
    const tipoRaw = txts.find(s => TIPO_MAP[s.toUpperCase().replace(/[Á]/g,'A').replace(/[É]/g,'E')])
    const tipo = tipoRaw ? TIPO_MAP[tipoRaw.toUpperCase().replace(/[Á]/g,'A').replace(/[É]/g,'E')] : undefined
    ...
    const sec = seccion ?? txts.find(s =>
      ...
      !/^\d[\d\s\-\.]{5,}$/.test(s) && s.length > 1 && !['L','M','S'].includes(s)
    )
...
function parseExcel(buffer: ArrayBuffer, filename: string) {
  ...
  for (const sname of wb.SheetNames) {
    ...
    if (upper === 'CONTACTO') {
      ...
      const PARENTS = new Set(['PAREJA','ESPOSO','ESPOSA','MADRE','PADRE','HERMANO','HERMANA','HIJO','HIJA','AMIGO','AMIGA'])
    ...
    if (ROOMS.has(upper)) {
      const rows = XLSX.utils.sheet_to_json<unknown[]>(ws, {header:1,defval:null})
      let hRow = -1
      for (let i=0;i<rows.length;i++) { if ((rows[i] as unknown[]).some(v=>/cuarto\s*\d*/i.test(String(v??'')))) {hRow=i;break} }
      if (hRow>=0) {
  ...
  for (const [nom, info] of infoMap) {
    if (!merged.has(nom) && looksLikeName(nom)) {
      merged.set(nom, {nombre:nom,estado:'activo',es_coordinador:false,es_operador:false,
        total_pagado:0,total_costo:0,saldo_pendiente:0,abonos:[],
        contacto:contactoMap.get(nom),...(info as object)})
    }
  }
...
export default function ImportarClient({ viajesExistentes, userId }: Props) {
  ...
  async function processFile(index: number) {
    ...
    try {
      const buf = await fs.file.arrayBuffer()
      const data = parseExcel(buf, fs.file.name)
      ...
      if (res.ok) {
        setFiles(prev=>prev.map((f,i)=>i===index?{...f,status:'done',
          message:`${result.viajeros} viajeros · ${result.habitaciones} cuartos importados`,
          viajeId:result.viajeId}:f))
      } else {
  ...
  async function processAll() {
    ...
    for (const idx of files.map((f,i)=>f.status==='pending'?i:-1).filter(i=>i>=0)) await processFile(idx)
    ...
    <div className="grid grid-cols-3 gap-6">
      <div className="col-span-2 space-y-6">
        <div onDragOver={e=>{e.preventDefault();setIsDragging(true)}} onDragLeave={()=>setIsDragging(false)}
          ...
          <h3 className="font-semibold text-gray-700 text-lg mb-1">{isDragging?'Suelta aquí':'Arrastra tus archivos Excel'}</h3>
          ...
          <div className="card overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <div className="flex gap-3 items-center">
                ...
                {pending>0&&<button onClick={processAll} disabled={isProcessing} className="btn-primary">
                ...
                <div key={i} className="px-5 py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-xs mt-0.5 ${fs.status==='done'?'text-green-600':fs.status==='error'?'text-red-500':fs.status==='processing'?'text-brand-500':'text-gray-400'}`}>
                      {fs.status==='pending'&&`${(fs.file.size/1024).toFixed(0)} KB`}
                      {fs.status==='processing'&&'Procesando...'}
                      {(fs.status==='done'||fs.status==='error')&&fs.message}
                    </p>
                  </div>
                  <div className="flex gap-2 items-center">
                    {fs.status==='done'&&fs.viajeId&&<Link href={`/viajes/${fs.viajeId}`} className="text-xs text-brand-600">Ver →</Link>}
                    {fs.status==='pending'&&<button onClick={()=>processFile(i)} className="btn-secondary text-xs py-1">Importar</button>}
                    <button onClick={()=>setFiles(prev=>prev.filter((_,j)=>j!==i))} className="p-1.5 text-gray-300 hover:text-red-400"><X className="w-3.5 h-3.5"/></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-2">
            {viajesExistentes.map(v=>(
              <Link key={v.id} href={`/viajes/${v.id}`} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-gray-50 group">
                <div className="w-7 h-7 bg-brand-100 rounded-lg flex items-center justify-center"><Bus className="w-3.5 h-3.5 text-brand-600"/></div>
                <div className="flex-1 min-w-0">
