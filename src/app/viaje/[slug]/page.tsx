...
export default async function ViajePage({ params }: Props) {
  ...
  const precioMin = Math.min(...Object.values(precios).filter((p: number) => p > 0))
...
function PrecioCard({ tipo, personas, camas, precio, color }: {
  tipo: string
  personas: string
  camas: string
  precio: number
  ...
  return (
    <div className={`bg-gradient-to-br ${color} p-6 rounded-2xl text-white transform hover:scale-105 transition-transform`}>
      <div className="text-center">
        <div className="text-4xl mb-3">
          {tipo === 'Cuádruple' && '🛏️🛏️🛏️'}
          {tipo === 'Triple' && '🛏️🛏️'}
          {tipo === 'Doble' && '🛏️'}
        </div>
        <h3 className="text-2xl font-bold mb-2">{tipo}</h3>
        <p className="text-sm opacity-90 mb-4">{personas}<br/>{camas}</p>
